import { TrendingUp, Package, Coins, Zap, Lock, Building2, Lightbulb } from "lucide-react";
import React from 'react';

export default function RevisionVisualizer() {
  const revisions = [
    { term: 'Pip', role: 'Movement', desc: 'Smallest standard unit of price change.', icon: <TrendingUp className="w-5 h-5 text-emerald-400" />, formula: '0.0001 (standard) / 0.01 (JPY) / 0.1 (Gold)' },
    { term: 'Lot', role: 'Size', desc: 'Volume size of your trade contract.', icon: <Package className="w-5 h-5 text-amber-400" />, formula: 'Standard (100k) / Mini (10k) / Micro (1k)' },
    { term: 'Spread', role: 'Cost', desc: 'Difference between Buy (Ask) and Sell (Bid) price.', icon: <Coins className="w-5 h-5 text-brandGold" />, formula: 'Spread = Ask - Bid (Broker transaction fee)' },
    { term: 'Leverage', role: 'Power', desc: 'Borrowed trading power provided by your broker.', icon: <Zap className="w-5 h-5 text-sky-400" />, formula: '1:100 leverage allows $100 to control $10,000' },
    { term: 'Margin', role: 'Deposit', desc: 'Required collateral held to open a leveraged position.', icon: <Lock className="w-5 h-5 text-purple-400" />, formula: 'Margin = Position Size ÷ Leverage Ratio' },
    { term: 'Broker', role: 'Middleman', desc: 'The company providing access to execute your trades.', icon: <Building2 className="w-5 h-5 text-blue-400" />, formula: 'Provides platform, liquidity access, and quotes' }
  ];
    ...
    <div className="w-full space-y-8 animate-fadeIn">
      ...
      <div className="bg-brandGold/5 border border-brandGold/20 rounded-2xl p-6 md:p-8 max-w-3xl mx-auto text-center relative overflow-hidden">
        ...
        <h4 className="text-brandGold font-bold font-display uppercase tracking-widest text-xs mb-4">
          <span className="flex items-center justify-center gap-2"><Lightbulb className="w-4 h-4 text-brandGold" /> Easy Student Memory Trick</span>
        </h4>
